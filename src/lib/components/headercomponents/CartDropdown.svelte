...
<div bind:this={dropdownElement} class="relative">
	...
	{#if isOpen && isBrowser && $currentUser}
		...
		>
			...
			<div class="border-b border-gray-100 px-5 py-3.5">
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-2.5">
						<h3 class="truncate text-base font-semibold text-gray-900">ショッピングカート</h3>
						{#if $cartStats.totalItems > 0}
							<span
								class="rounded-full bg-gray-900 px-2 py-0.5 text-xs font-medium whitespace-nowrap text-white"
							>
					...
					<button
						class="flex-shrink-0 rounded-full p-1.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600"
			...
			<!-- 商品列表 -->
			<div class="custom-scrollbar max-h-[350px] overflow-y-auto">
				{#if $cart.loading}
									...
									<div class="flex min-w-0 flex-1 flex-col">
										<div class="mb-1.5 flex items-start justify-between gap-2">
											<div class="min-w-0 flex-1">
												<h4
													class="mb-0.5 line-clamp-2 text-sm font-semibold text-gray-900 group-hover:text-gray-600"
												...
												{#if item.skuInfo}
													<div class="flex flex-wrap gap-1">
														{#each Object.entries(item.skuInfo.specs) as [key, value]}
															<span
																class="max-w-[120px] truncate font-mono text-[12px] text-gray-500"
																title="{key}:{value}"
															>
																{key}:{value}
															</span>
										...
										<div class="mt-auto flex items-end justify-between gap-2">
											...
											<div class="flex min-w-0 flex-col">
												<div class="flex flex-wrap items-baseline gap-1.5">
													<span class="text-sm font-bold whitespace-nowrap text-gray-900">
														{formatPrice(item.product.price)}
													</span>
													{#if item.product.original_price && item.product.original_price > item.product.price}
														<span class="text-xs whitespace-nowrap text-gray-400 line-through">
															{formatPrice(item.product.original_price)}
														</span>
													{/if}
													<div class="text-[10px] whitespace-nowrap text-gray-500">1個あたり</div>
												</div>
											</div>
					...
					<div class="mb-3 rounded-lg bg-gray-50 p-3">
						<div class="mb-1.5 flex items-center justify-between">
							...
							<span class="text-base font-bold text-gray-900"
								>{formatPrice($cartStats.subtotal)}</span
							>
						</div>
