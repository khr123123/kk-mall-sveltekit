<script lang="ts">
	// Props
	export let appName: string = 'K. Portfolio';
	export let qrCodeUrl: string = '/QR_code.png'; // 二维码图片 URL
	export let downloadLinks = {
		ios: 'https://apps.apple.com/app/...',
		android: 'https://play.google.com/store/apps/...'
	};

	// State
	let showDropdown = false;
	let dropdownTimeout: number;

	function openDropdown() {
		clearTimeout(dropdownTimeout);
		showDropdown = true;
	}

	function closeDropdown() {
		dropdownTimeout = window.setTimeout(() => {
			showDropdown = false;
		}, 150);
	}
</script>

<div
	class="qrcode-dropdown-container relative"
	onmouseenter={openDropdown}
	onmouseleave={closeDropdown}
>
	<button
		class="qrcode-button hidden min-w-[150px] items-center gap-2 rounded-lg bg-white px-3 py-2 font-mono text-xs text-gray-700 transition hover:bg-gray-100 hover:text-gray-900 lg:flex"
		aria-haspopup="true"
		aria-expanded={showDropdown}
	>
		<img src="/svgs/二维码.svg" alt="QRコード" class="h-6 w-6 shrink-0" />
		<div class="flex flex-col overflow-hidden text-left leading-tight">
			<span class="truncate text-xs text-gray-500">モバイルアプリを</span>
			<span class="truncate text-xs font-medium">ダウンロード</span>
		</div>
	</button>

	<!-- QR Code Dropdown - PocketBase Style -->
	{#if showDropdown}
		<div
			class="qrcode-dropdown absolute top-full right-0 mt-2 w-72 rounded-xl border border-gray-200 bg-white p-5 shadow-2xl"
			onmouseenter={openDropdown}
			onmouseleave={closeDropdown}
			role="menu"
		>
			<!-- Header -->
			<div class="mb-4 text-center">
				<h3 class="text-base font-semibold text-gray-900">{appName}</h3>
				<p class="mt-1 text-xs text-gray-500">スマートフォンでスキャン</p>
			</div>

			<!-- QR Code -->
			<div class="mb-4 flex justify-center rounded-lg bg-gray-50 p-4">
				<div class="qr-code-wrapper rounded-lg bg-white p-3 shadow-sm">
					<img src={qrCodeUrl} alt="QR Code" class="h-32 w-32" />
				</div>
			</div>

			<!-- Download Links -->
			<div class="space-y-2">
				<a
					href={downloadLinks.ios}
					target="_blank"
					rel="noopener noreferrer"
					class="download-link flex items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition-all hover:border-gray-300 hover:bg-gray-50 hover:shadow-sm"
				>
					<svg class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
						<path
							d="M18.71 19.5C17.88 20.74 17 21.95 15.66 21.97C14.32 22 13.89 21.18 12.37 21.18C10.84 21.18 10.37 21.95 9.1 22C7.79 22.05 6.8 20.68 5.96 19.47C4.25 17 2.94 12.45 4.7 9.39C5.57 7.87 7.13 6.91 8.82 6.88C10.1 6.86 11.32 7.75 12.11 7.75C12.89 7.75 14.37 6.68 15.92 6.84C16.57 6.87 18.39 7.1 19.56 8.82C19.47 8.88 17.39 10.1 17.41 12.63C17.44 15.65 20.06 16.66 20.09 16.67C20.06 16.74 19.67 18.11 18.71 19.5ZM13 3.5C13.73 2.67 14.94 2.04 15.94 2C16.07 3.17 15.6 4.35 14.9 5.19C14.21 6.04 13.07 6.7 11.95 6.61C11.8 5.46 12.36 4.26 13 3.5Z"
						/>
					</svg>
					<span>App Store</span>
				</a>

				<a
					href={downloadLinks.android}
					target="_blank"
					rel="noopener noreferrer"
					class="download-link flex items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition-all hover:border-gray-300 hover:bg-gray-50 hover:shadow-sm"
				>
					<svg class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
						<path
							d="M3,20.5V3.5C3,2.91 3.34,2.39 3.84,2.15L13.69,12L3.84,21.85C3.34,21.6 3,21.09 3,20.5M16.81,15.12L6.05,21.34L14.54,12.85L16.81,15.12M20.16,10.81C20.5,11.08 20.75,11.5 20.75,12C20.75,12.5 20.53,12.9 20.18,13.18L17.89,14.5L15.39,12L17.89,9.5L20.16,10.81M6.05,2.66L16.81,8.88L14.54,11.15L6.05,2.66Z"
						/>
					</svg>
					<span>Google Play</span>
				</a>
			</div>

			<!-- Footer Info -->
			<div class="mt-4 border-t border-gray-100 pt-3 text-center">
				<p class="text-xs text-gray-400">
					<svg
						class="mr-1 inline-block h-3 w-3"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
						<path
							stroke-linecap="round"
							stroke-linejoin="round"
							stroke-width="2"
							d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"
						/>
					</svg>
					安全なダウンロード
				</p>
			</div>
		</div>
	{/if}
</div>

<style>
	/* PocketBase-inspired styles */
	.qrcode-dropdown {
		animation :dropdownFade 0.2s ease-out;
		z-index: 50;
	}

	@keyframes dropdownFade {
		from {
			opacity: 0;
			transform: translateY(-8px) scale(0.96);
		}
		to {
			opacity: 1;
			transform: translateY(0) scale(1);
		}
	}

	.qr-code-wrapper {
		border: 2px solid #f3f4f6;
	}

	.download-link {
		position: relative;
		overflow: hidden;
	}

	.download-link::before {
		content: '';
		position: absolute;
		top: 0;
		left: -100%;
		width: 100%;
		height: 100%;
		background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.5), transparent);
		transition :left 0.5s;
	}

	.download-link:hover::before {
		left: 100%;
	}

	@media (max-width: 1280px) {
		.qrcode-button {
			min-width: 140px;
		}
	}
</style>
