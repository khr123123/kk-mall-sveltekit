...
const createCartStore = () => {
	...
	const setQuantity = async (itemId: string, quantity: number): Promise<void> => {
		...
		update((state) => {
			...
			return {
				...state,
				items: state.items.map((i) => (i.id === itemId ? { ...i, quantity: finalQuantity } : i))
			};
		});
	};
	...
	const toggleSelect = async (itemId: string): Promise<void> => {
		update((state) => {
			...
			return {
				...state,
				items: state.items.map((i) => (i.id === itemId ? { ...i, selected } : i))
			};
		});
	};
	...
	const toggleSelectAll = async (selected: boolean): Promise<void> => {
		...
		await Promise.all(items.map((item) => cartAPI.toggleSelectItem(item.id, selected)));
...
export const cartStats = derived(cart, ($cart) => {
	const selectedItems = $cart.items.filter((i) => i.selected && i.product.in_stock);
	...
	return {
		...
		discount,
		// 是否全选
		allSelected: $cart.items.length > 0 && selectedItems.length === $cart.items.length,
		// 是否有不可用商品
