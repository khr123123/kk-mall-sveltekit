<script lang="ts">
    ...
    async function findFavoriteRecord(brandId: string) {
        try {
            const userId = pb.authStore.record!.id;

            // 查找包含该商品ID的收藏记录
            const records = await pb.collection('favorites').getFullList({
                filter: `user = "${userId}" && brands_id ~ "${brandId}"`
            });
    ...
    async function followBrand() {
        try {
            // 先检查是否已有收藏记录
            const existingRecord = await findFavoriteRecord(brandId);

            if (existingRecord) {
                // 如果记录已存在，将新的product_id添加到数组中
                const updatedProductIds = [...(existingRecord.product_id || []), brandId];
                await pb.collection('favorites').update(existingRecord.id, {
                    brands_id: updatedProductIds
                });
            } else {
                const userId = pb.authStore.record!.id;
                // 创建新的收藏记录
    ...
    async function unfollowBrand() {
        console.log('フォローを解除:', brand?.name);
        try {
            const existingRecord = await findFavoriteRecord(brandId);
            if (existingRecord) {
                // 从product_id数组中移除该商品ID
                const currentProductIds = existingRecord.product_id || [];
                const updatedProductIds = currentProductIds.filter((id: string) => id !== brandId);
