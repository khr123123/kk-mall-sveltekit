<script lang="ts">
    ...
    let loginPromptTimeout: number | null = null;
...
{#if showLoginPrompt}
    <div class="fixed inset-x-0 top-4 z-50 flex justify-center px-4">
        <div class="animate-in slide-in-from-top-5 fade-in rounded-lg bg-gray-900 px-4 py-3 text-sm text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                    </svg>
                    <span>お気に入り機能を使用するには<a href="/login" class="ml-1 underline hover:no-underline">ログイン</a>してください</span>
                </div>
                <button onclick={hideLoginPrompt} class="ml-4 text-gray-300 hover:text-white">
                    ✕
                </button>
            </div>
        </div>
