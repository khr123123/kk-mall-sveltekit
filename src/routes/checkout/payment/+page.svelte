...
<script lang="ts">
    ...
    import {checkMd as check, link, lockMd as lock, paypay, spinner} from '$lib/icons/svgs';
    ...
    async function createPayPayOrder() {
        ...
        try {
            ...
            // ✅ 获取订单真实金额
            const orderAmount = orderData?.total || orderData?.totalAmount || 0;

            if (orderAmount <= 0) {
                throw new Error('無効な注文金額です');
            }

            // ✅ 调用后端API创建PayPay订单 - 使用真实金额
            ...
            if (qrCode) {
                ...
                console.log('✅ 订单金额:', formatPrice(orderAmount));
                    ...
                    <div class="mt-8 rounded-lg bg-gray-50 p-4 text-left">
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            <div class="flex justify-between">
                                ...
                                <!-- ✅ 显示真实订单金额 -->
                                <span class="font-bold text-gray-900">{formatPrice(orderData.total || orderData.totalAmount)}</span>
                            </div>
                            <div class="flex justify-between">
