...
<script lang="ts">
    ...
    let twoFactorForm = $state({
        step: 'intro' as 'intro' | 'setup' | 'verify' | 'complete',
        verificationCode: '',
    ...
    async function saveAddress() {
        ...
        if (result.success) {
            ...
            showAlert({
                title: '成功',
                message: `住所を${editingAddress ? '更新' : '作成'}しました`,
                type: 'success'
            });
        } else {
            showAlert({
    ...
    // ============ 订单详情管理 ============
    function openOrderDetailModal(order: Order) {
        selectedOrder = order;
    ...
    async function verifyTwoFactorCode() {
        if (!twoFactorForm.verificationCode || twoFactorForm.verificationCode.length !== 6) {
            showAlert({
                title: 'エラー',
    ...
    function copyBackupCodes() {
        const text = twoFactorForm.backupCodes.join('\n');
    ...
    async function deleteAccount() {
        const confirmed = await showConfirm({
            title: '警告',
            message: 'アカウントを削除すると、すべてのデータが完全に削除されます。この操作は元に戻せません。続行しますか？',
            type: 'danger',
            confirmText: '削除',
            cancelText: 'キャンセル'
        });
                                    ...
                                    {#each addresses as address}
                                        <div class="address-card">
                                            ...
                                            <div class="space-y-1 text-sm text-[#4a5568]">
                                                ...
                                                <p>{formatters.phone(address.phone)}</p>
                                                <p>{formatters.postalCode(address.postal_code)}</p>
                                ...
                                <div class="border-b border-[#e0e0e0] pb-6">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <label class="setting-item" onclick={() => toggleNotification('newProducts')}>
                                            ...
                                            <input type="checkbox" class="toggle"
                                                   checked={settingsForm.notifications.newProducts}/>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <div class="border-b border-[#e0e0e0] pb-6">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <button class="setting-btn" onclick={openTwoFactorModal}>
                                            <span>{settingsForm.isTwoFactorEnabled ? '2段階認証を管理' : '2段階認証を設定'}</span>
                                            {@html arrow}
                                        </button>
                                    </div>
                                </div>
        ...
        <div class="modal-overlay" onclick={closeOrderDetailModal}>
            <div class="modal-content" onclick={(e) => e.stopPropagation()}>
                <div class="mb-4 flex items-center justify-between">
                    ...
                    <button class="icon-btn" onclick={closeOrderDetailModal}>
                        {@html close}
                    </button>
                </div>
                    ...
                    <div class="rounded-lg bg-gray-50 p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <span class="ml-2 status-badge {getStatusClass(selectedOrder.status)}">
                                    {getStatusLabel(selectedOrder.status)}
                                </span>
                            </div>
                            <div class="text-sm text-gray-600">
                                注文日: {new Date(selectedOrder.order_date).toLocaleDateString('ja-JP')}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="rounded-lg border border-gray-200 p-4">
                            ...
                            <p class="text-sm text-gray-600">
                                {selectedOrder.shipping_address || '住所情報がありません'}
                            </p>
                        </div>
                        <div class="rounded-lg border border-gray-200 p-4">
                            ...
                            <p class="text-sm text-gray-600">
                                {selectedOrder.payment_method === 'credit_card' ? 'クレジットカード' :
                                    selectedOrder.payment_method === 'paypay' ? 'PayPay' :
                                        selectedOrder.payment_method === 'cod' ? '代金引換' : 'その他'}
                            </p>
                        </div>
                    </div>
                            ...
                            <div class="max-h-96 overflow-y-auto relative">
                                <table class="w-full min-w-max">
                                    ...
                                    <tr class="text-left text-sm">
                                        <th class="py-3 px-4 font-semibold text-gray-700 border-b">商品</th>
                                        <th class="py-3 px-4 font-semibold text-gray-700 border-b text-right">価格</th>
                                        <th class="py-3 px-4 font-semibold text-gray-700 border-b text-center">数量</th>
                                        <th class="py-3 px-4 font-semibold text-gray-700 border-b text-right">小計</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    {#each selectedOrder.items as item}
                                        <tr class="border-b border-gray-100 hover:bg-gray-25 transition-colors">
                                            <td class="py-3 px-4">
                                                <div class="flex items-center gap-3">
                                                    <img
                                            ...
                                            <td class="py-3 px-4 text-sm font-semibold text-gray-800 text-right">
                ...
                <div class="mt-6 flex justify-end">
                    <button class="btn-secondary" onclick={closeOrderDetailModal}>
                        閉じる
                    </button>
                </div>
            </div>
            ...
            <div class="modal-content" onclick={(e) => e.stopPropagation()}>
                ...
                >
                    ...
                    <div class="password-strength">
                        ...
                        <div class="strength-text text-xs text-[#718096]">
                            パスワードの強度: 弱い
                        </div>
                    </div>
    ...
    {#if showTwoFactorModal}
        ...
        <div class="modal-overlay" onclick={closeTwoFactorModal}>
            <div class="modal-content" onclick={(e) => e.stopPropagation()}>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex gap-3">
                                <button
                                        type="button"
                                        class="btn-secondary flex-1"
                                        onclick={() => twoFactorForm.step = 'intro'}
                                >
                                    戻る
                                </button>
                                <button
                                        type="button"
                                        class="btn-primary flex-1"
                                        onclick={() => twoFactorForm.step = 'verify'}
                                >
                                    次へ
                                </button>
                            </div>
                ...
                {:else if twoFactorForm.step === 'verify'}
                    <div class="space-y-6">
                        <div class="text-center">
                            <h4 class="mb-2 text-lg font-semibold text-[#1a1a1a]">
                                確認コードを入力
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex gap-3">
                                <button
                                        type="button"
                                        class="btn-secondary flex-1"
                                        onclick={() => twoFactorForm.step = 'setup'}
                                >
                                    戻る
                                </button>
                                <button
                                        ...
                                        disabled={twoFactorForm.verificationCode.length !== 6}
                                >
                                    確認
                                </button>
                            </div>
                        ...
                        {#if !twoFactorForm.showBackupCodes}
                            <div class="rounded-lg bg-yellow-50 p-4">
                                ...
                                <button
                                        ...
                                        onclick={() => twoFactorForm.showBackupCodes = true}
                                >
                                    バックアップコードを表示
                                </button>
                            </div>
                            ...
                            <div class="rounded-lg border border-gray-200 p-4">
                                <div class="mb-3 flex items-center justify-between">
                                    ...
                                    <button
                                            ...
                                            onclick={() => twoFactorForm.showBackupCodes = false}
                                    >
                                        閉じる
                                    </button>
                                </div>
