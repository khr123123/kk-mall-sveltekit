...
<script lang="ts">
	...
	async function deleteAccount() {
		const confirmed = await showConfirm({
			title: '警告',
			message:
				'アカウントを削除すると、すべてのデータが完全に削除されます。この操作は元に戻せません。続行しますか？',
			type: 'danger',
			confirmText: '削除',
			cancelText: 'キャンセル'
		});
								...
								<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
									{#each favorites as favorite}
										<div class="product-card">
											...
											<div class="flex gap-2">
												<button class="btn-primary flex-1" onclick={() => addToCart(favorite.id)}>
													{@html cart}
													<span>カートに入れる</span>
												</button>
												<button
							...
							<div class="space-y-6">
								...
								<div>
									...
									<div class="space-y-2">
										...
										<button
											class="setting-btn text-[#e53e3e] hover:bg-[#fff5f5]"
					...
					<div class="rounded-lg bg-gray-50 p-4">
						<div class="flex items-center justify-between">
							<div>
								...
								<span class="status-badge ml-2 {getStatusClass(selectedOrder.status)}">
						...
						<div class="rounded-lg border border-gray-200 p-4">
							...
							<p class="text-sm text-gray-600">
								{selectedOrder.payment_method === 'credit_card'
									? 'クレジットカード'
									: selectedOrder.payment_method === 'paypay'
										? 'PayPay'
								...
								<table class="w-full min-w-max">
									<thead class="sticky top-0 z-10 bg-gray-50">
										<tr class="text-left text-sm">
											...
											<th class="border-b px-4 py-3 text-right font-semibold text-gray-700">価格</th
											>
											<th class="border-b px-4 py-3 text-center font-semibold text-gray-700"
												>数量</th
											>
											<th class="border-b px-4 py-3 text-right font-semibold text-gray-700">小計</th
											>
										</tr>
									</thead>
									<tbody>
		...
		<div class="modal-overlay" onclick={closeTwoFactorModal}>
			<div class="modal-content" onclick={(e) => e.stopPropagation()}>
				...
				{:else if twoFactorForm.step === 'setup'}
					<div class="space-y-6">
						<div class="text-center">
							<h4 class="mb-4 text-lg font-semibold text-[#1a1a1a]">認証アプリを設定</h4>
						</div>
						...
						<div class="space-y-4">
							<div>
								...
								<input
									...
									maxlength="6"
									placeholder="000000"
									class="input-field text-center font-mono text-2xl tracking-widest"
								/>
							</div>
